...
bool deleteRowsFromTable(const string& tableName, const string& column, const string& value, const TableJson& json_table) {
    ...
    // Ищем все CSV файлы
    while (true) {
        fs::path filePath = fs::path("/home/pract1/") / json_table.Name / tableName / (to_string(amountCsv) + ".csv");
    ...
    for (size_t iCsv = 1; iCsv < amountCsv; iCsv++) {
        string filePath = "/home/pract1/" + json_table.Name + "/" + tableName + "/" + (to_string(iCsv) + ".csv");
