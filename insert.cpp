...
bool isloker(const string& tableName, const string& schemeName) {
    string baseDir = "/home/pract1/" + schemeName + "/" + tableName;
...
int findCsvFileCount(const TableJson& json_table, const string& tableName) {
    ...
    while (true) {
        string csvFile = "/home/pract1/" + json_table.Name + "/" + tableName + "/" + (to_string(csvNumber) + ".csv");
...
void createNewCsvFile(const string& baseDir, const string& tableName, int& csvNumber, const TableJson& tableJson) {
    ...
    int maxRowsPerFile = tableJson.table_size;
...
void insert(const string& command, TableJson& json_table) {
    ...
    int currentPK;
    string PKFile = "/home/pract1/" + json_table.Name + "/" + tableName + "/" + (tableName + "_pk_sequence.txt");
    ...
    string baseDir = "/home/pract1/" + json_table.Name;
